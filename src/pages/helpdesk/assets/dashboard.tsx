...
const AssetDashboard = () => {
  ...
  const { data: assets = [], isLoading: assetsLoading, refetch: refetchAssets, dataUpdatedAt } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("itam_assets")
        .select("id, asset_id, asset_tag, name, status, is_active, purchase_price, purchase_date, warranty_expiry, expected_return_date, checked_out_to, assigned_to, checked_out_at, created_at, updated_at, custom_fields, category:itam_categories(id, name)")
        .eq("is_active", true)
        .limit(5000);
  ...
  // ── Stats (memoized) ──
  const { totalAssets, activeAssets, availableAssets, totalValue, hasMixedCurrencies, checkedOutCount, underRepairCount, disposedCount, fiscalYearValue, fiscalYearPurchases } = useMemo(() => {

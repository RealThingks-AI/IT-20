...
} from "@/components/helpdesk/assets/ManageDashboardDialog";
import {
  Package,
  DollarSign,
  CheckCircle2,
...
const AssetDashboard = () => {
  ...
  const { data: assets = [], isLoading: assetsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("itam_assets")
        .select("*, category:itam_categories(id, name)")
        .eq("is_active", true);
      return data || [];
    },
  });
  ...
  const { data: maintenanceDue = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("itam_repairs")
        .select("*, asset:itam_assets(id, name, asset_tag)")
        .eq("status", "pending")
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4 md:p-6 space-y-6">
        {/* Header with Manage Dashboard */}
        <div className="flex items-center justify-between animate-fade-in">
          <h1 className="text-xl font-semibold tracking-tight">Asset Dashboard</h1>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setManageDialogOpen(true)}
            className="gap-1.5 h-8 transition-all duration-200 hover:shadow-sm"
          >
            <Settings2 className="h-4 w-4" />
            <span className="hidden sm:inline">Manage Dashboard</span>
          </Button>
        </div>
        ...
        {assetsLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <Skeleton key={i} className="h-[100px] animate-pulse" />
            ))}
          </div>
        ) : (
          <div className={`grid grid-cols-1 sm:grid-cols-2 ${getGridCols(preferences.columns)} gap-4`}>
              ...
              switch (widget.id) {
                case "activeAssets":
                  return (
                    <AssetStatCard
                      key={widget.id}
                      title="Number of Active Assets"
                      value={activeAssets}
                      subtitle={`Total Assets: ${totalAssets}`}
                      icon={Package}
                ...
                case "availableAssets":
                  return (
                    <AssetStatCard
                      ...
                      subtitle={`Value: ${formatCurrency(assets.filter(a => a.status === "available").reduce((sum, a) => sum + (parseFloat(String(a.purchase_price || 0)) || 0), 0))}`}
                      icon={CheckCircle2}
                      iconBgColor="bg-green-500"
                      iconColor="text-white"
                      onClick={() => navigate("/assets/allassets?status=available")}
                      animationDelay={animationDelay}
                    />
                  );
                case "assetValue":
                ...
                case "fiscalPurchases":
                  return (
                    <AssetStatCard
                      ...
                      value={formatCurrency(fiscalYearValue)}
                      subtitle={`${fiscalYearPurchases.length} assets purchased`}
                      icon={ShoppingCart}
                ...
                case "disposed":
                  return (
                    <AssetStatCard
                      ...
                      iconColor="text-white"
                      onClick={() => navigate("/assets/allassets?status=disposed")}
                      animationDelay={animationDelay}
                    />
                  );
                case "contracts":
                  return (
                    <AssetStatCard
                      ...
                      iconColor="text-white"
                      onClick={() => navigate("/assets/lists/contracts")}
                      animationDelay={animationDelay}
                    />
                  );
                default:
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {preferences.showChart && (
            <Card className="lg:col-span-2 animate-fade-in transition-all duration-200 hover:shadow-md" style={{ animationDelay: "100ms", animationFillMode: "backwards" }}>
            ...
            <Card className={cn(
              ...
              <CardContent className="p-0">
                <Tabs defaultValue={feedTabs[0]?.id || "checkedin"} className="w-full">
                  <TabsList className={`w-full grid rounded-none border-b`} style={{ gridTemplateColumns: `repeat(${Math.min(feedTabs.length, 3)}, 1fr)` }}>
                      ...
                      <div className="divide-y">
                        {recentCheckins.map((checkin) => (
                          <div
                            ...
                            onClick={() => navigate(`/assets/detail/${checkin.asset_id}`)}
                          >
                      ...
                      <div className="divide-y">
                        {recentCheckouts.map((checkout) => (
                          <div
                            key={checkout.id}
                            className="p-3 hover:bg-accent cursor-pointer transition-colors duration-150"
                            onClick={() => navigate(`/assets/detail/${checkout.asset_id}`)}
                          >
                            <div className="flex items-center justify-between gap-2">
                              <div className="min-w-0 flex-1">
        ...
        {preferences.showAlerts && (
          <Card className="animate-fade-in transition-all duration-200 hover:shadow-md" style={{ animationDelay: "200ms", animationFillMode: "backwards" }}>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                {/* Assets Due */}
                <div
                  className="flex items-center gap-3 p-3 rounded-lg border border-red-200 bg-red-50/50 dark:bg-red-950/20 dark:border-red-900 cursor-pointer hover:border-red-400 hover:shadow-sm transition-all duration-200 hover:-translate-y-0.5"
                  onClick={() => navigate("/assets/alerts?type=overdue")}
                >
                ...
                {/* Maintenance Due */}
                <div
                  className="flex items-center gap-3 p-3 rounded-lg border border-green-200 bg-green-50/50 dark:bg-green-950/20 dark:border-green-900 cursor-pointer hover:border-green-400 hover:shadow-sm transition-all duration-200 hover:-translate-y-0.5"
                  onClick={() => navigate("/assets/alerts?type=maintenance")}
                >
                ...
                {/* Contracts Pending */}
                <div
                  className="flex items-center gap-3 p-3 rounded-lg border border-orange-200 bg-orange-50/50 dark:bg-orange-950/20 dark:border-orange-900 cursor-pointer hover:border-orange-400 hover:shadow-sm transition-all duration-200 hover:-translate-y-0.5"
                  onClick={() => navigate("/assets/alerts?type=contracts")}
                >
                ...
                {/* Warranty Pending */}
                <div
                  className="flex items-center gap-3 p-3 rounded-lg border border-purple-200 bg-purple-50/50 dark:bg-purple-950/20 dark:border-purple-900 cursor-pointer hover:border-purple-400 hover:shadow-sm transition-all duration-200 hover:-translate-y-0.5"
                  onClick={() => navigate("/assets/alerts?type=warranty")}
                >

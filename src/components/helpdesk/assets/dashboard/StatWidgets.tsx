...
import {
  ...
  Calendar, Clock, KeyRound, Trash2,
} from "lucide-react";
import type { DashboardAsset, OverdueAssignment, LicenseRecord } from "./types";
...
export function StatWidgets({
  ...
  if (assetsLoading) {
    return (
      <div className={cn("grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3", gridColsClass)}>
    ...
    <div className={cn("grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3", gridColsClass)}>
      {enabledWidgets.map((widget, index) => {
        ...
        switch (widget.id) {
          ...
          case "licenses":
            return <AssetStatCard key={widget.id} title="Licenses" value={activeLicenses.length} subtitle={`${expiringLicenses.length} expiring`} icon={KeyRound} iconBgColor="bg-indigo-500" iconColor="text-white" onClick={() => navigate("/assets/advanced?tab=licenses")} animationDelay={d} />;
